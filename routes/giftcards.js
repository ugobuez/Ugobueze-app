...
const uploadToCloudinary = (fileBuffer) => {
  return new Promise((resolve, reject) => {
    const uploadStream = cloudinary.uploader.upload_stream(
      { folder: "redeemed_cards", resource_type: "auto", timeout: 60000 },
      (error, result) => {
        if (error) {
...
// Create gift card
router.post("/", authenticateToken, checkMongoConnection, async (req, res) => {
  try {
    const { name, brand, value, currency, image } = req.body;
    if (!name || !brand || !value || !currency || !image) return res.status(400).json({ error: "Validation error", message: "All fields are required" });
